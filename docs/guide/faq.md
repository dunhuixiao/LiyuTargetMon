...
- [为什么我导入了以后 Buff 图标还是不显示？](#为什么我导入了以后-buff-图标还是不显示)
- [如果我不喜欢倒计时、边框风格、右键点掉 Buff 功能，怎么办？](#如果我不喜欢倒计时边框风格右键点掉-buff-功能怎么办)
- [我该怎么样调整图标、文字大小？](#我该怎么样调整图标文字大小)
- [如果我想要添加自己单独的监控怎么办？](#如果我想要添加自己单独的监控怎么办)
- [为什么有些 Buff 没有加入监控？](#为什么有些-buff-没有加入监控)
- [Buff / Debuff 图标修改右对齐就不显示？](#buff--debuff-图标修改右对齐就不显示)
- [我该怎么调整技能 Buff 显示的顺序？](#我该怎么调整技能-buff-显示的顺序)
- [如果我想要单独对云剑斧进行重要 Buff 监控怎么办？](#如果我想要单独对云剑斧进行重要-buff-监控怎么办)
...
使用编辑器（推荐 VS Code、Notepad++ 等）用 `GBK` 编码格式打开 `.jx3dat` 文件，里面数据结构可以使用 `Lua` 或者 `JSON` 格式化查看，找到你想调整的 Buff，将该 Buff 调整到你想调整的位置，注意每个 Buff 的花括号作用范围。

:::tip VS Code 设置编码方法
1. 打开文件后，点击右下角的编码显示（如 UTF-8）
2. 选择「通过编码重新打开」
3. 选择「Chinese Simplified (GBK)」
:::
